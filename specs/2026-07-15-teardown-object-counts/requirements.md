...
- Gated teardown phases (not yet reached) get a read-only cache lookup for presence instead of being marked Unknown
...
- A COS mid-teardown shows accurate present counts for gated phases (via cache lookup, not Unknown status)
