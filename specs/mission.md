...
- **Independent usability** — the orchestration layer (COS/COSR) is usable without the resolution layer. Users and controllers can create COSRs directly.
...
- **ADR-driven** — follow the ADR-0001 architecture faithfully. COS/COSR/ClusterObjectSlice separation, phased rollout, and collision protection work as designed.
...
- **Template metadata is the extension point** — caller-specific metadata flows through `template.metadata` on the COS, not through new spec fields.
