Feature: COSR Active and Archived lifecycle behavior
  ...
  Scenario: Available condition flaps as per-phase assertions fail and recover
    Given a COSR with group "test" and revision 1
    And a phase "p1" with a ConfigMap "cm-p1" with assertion celExpression "!has(self.data) || self.data['fail'] != 'true'"
    And a phase "p2" with a ConfigMap "cm-p2" with assertion celExpression "!has(self.data) || self.data['fail'] != 'true'"
    And a phase "p3" with a ConfigMap "cm-p3" with assertion celExpression "!has(self.data) || self.data['fail'] != 'true'"
