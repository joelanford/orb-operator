Feature: COS multi-revision ownership handoffs within a group
  ...
  Scenario: Partially overlapping revisions transfer shared objects and retain unique objects
    ...
    When a COS with group "test" and revision 2
    And the phase "install" has a ConfigMap "cm-shared"
    And the phase "install" also has a ConfigMap "cm-new-only"
    And the COS is created and becomes Available
    Then the ConfigMap "cm-old-only" should have a controller owner reference to COS with group "test" and revision 1
    And the ConfigMap "cm-shared" should have a controller owner reference to COS with group "test" and revision 2
    And the ConfigMap "cm-new-only" should have a controller owner reference to COS with group "test" and revision 2
  ...
  Scenario: Non-contiguous revision numbers work correctly
    ...
    And the COS is created and becomes Available
    When a COS with group "test" and revision 5
    And the phase "install" has a ConfigMap "cm-gap"
    And the COS is created and becomes Available
    Then the ConfigMap "cm-gap" should have a controller owner reference to COS with group "test" and revision 5
  ...
  Scenario: Fully overlapping revisions supersede the predecessor
    ...
    And the phase "install" has a ConfigMap "cm-shared"
    And the COS is created and becomes Available
    Then the ConfigMap "cm-shared" should have a controller owner reference to COS with group "test" and revision 2
    And revision 1 should have condition "Available" with status "False" and reason "Superseded"

  Scenario: Predecessor COS recreates its deleted unique object
    Given a COS with group "test" and revision 1
    And a phase "install" with a ConfigMap "cm-old-only"
    And the phase "install" also has a ConfigMap "cm-shared"
    And the COS is created and becomes Available
