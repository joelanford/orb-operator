Feature: COD creates new revisions on template changes
  ...
  Scenario: Updating template spec creates a new revision
    ...
    Then a COS should exist with group "rev-spec" and revision 2
  ...
  Scenario: Updating template metadata creates a new revision
    ...
    Then a COS should exist with group "rev-meta" and revision 2
  ...
  Scenario: Multiple template changes produce monotonically increasing revisions
    ...
    Then a COS should exist with group "rev-multi" and revision 3
  ...
  Scenario: Template change during in-progress transition
    ...
    Then a COS should exist with group "rev-midtx" and revision 2
    ...
    Then a COS should exist with group "rev-midtx" and revision 3
    # Rev 3 eventually becomes available; rev 1 and 2 are archived
    And the COD "rev-midtx" should be Available
    And the COS with group "rev-midtx" and revision 1 should have lifecycleState "Archived"
    And the COS with group "rev-midtx" and revision 2 should have lifecycleState "Archived"

  Scenario: No new COS when template has not changed
    Given a COD named "rev-idempotent"
    And a phase "install" with a ConfigMap "cm-idempotent"
    When the COD is created
    Then a COS should exist with group "rev-idempotent" and revision 1
    ...
    Then the COS count for COD "rev-idempotent" should be 1
