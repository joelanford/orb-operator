Feature: COD enforces revisionHistoryLimit on archived COSs

  Scenario: Archived COSs beyond revisionHistoryLimit are pruned
    ...
    And the COD "hist-serial" should be Available
    # With limit 2, only the 2 highest-revision archived COSs are retained
    And the COS with group "hist-serial" and revision 1 should not exist

  Scenario: Active COS is never counted toward the limit
    Given a COD named "hist-active" with revisionHistoryLimit 0
    And a phase "install" with a ConfigMap "cm-hist-active"
    When the COD is created
    Then a COS should exist with group "hist-active" and revision 1
    # Even with limit 0, the active COS is preserved
    And the COS count for COD "hist-active" should be 1
  ...
  Scenario: Default revisionHistoryLimit of 5 applies
    ...
    And the COS with group "hist-default" and revision 1 should not exist

  Scenario: Lowering revisionHistoryLimit retroactively prunes archived COSs
    ...
    And the COS with group "hist-lower" and revision 1 should not exist
    # Lower limit to 2: revisions 2-4 should be pruned retroactively
    When the COD "hist-lower" revisionHistoryLimit is set to 2
    Then the COS with group "hist-lower" and revision 2 should not exist
    And the COS with group "hist-lower" and revision 3 should not exist
    And the COS with group "hist-lower" and revision 4 should not exist
    # Revisions 5-6 (archived) and 7 (active) remain
    And a COS should exist with group "hist-lower" and revision 5
    And a COS should exist with group "hist-lower" and revision 6
    And a COS should exist with group "hist-lower" and revision 7
